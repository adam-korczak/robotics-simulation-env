...
def parse_args():
    parser = argparse.ArgumentParser(description='Particle Filter for Kidnapped Robot Localization')
...
def initialize_particles_randomly(map_size, num_particles):
    x_positions = np.random.uniform(0, map_size[0], num_particles)
    y_positions = np.random.uniform(0, map_size[1], num_particles)
    thetas = np.random.uniform(-np.pi, np.pi, num_particles)
    return np.column_stack((x_positions, y_positions, thetas))
...
def main():
    ...
    map_size = [max(map_landmarks[:, 0]), max(map_landmarks[:, 1])]
    particles = initialize_particles_randomly(map_size, args.num_particles)
